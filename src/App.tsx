...
import {Header} from './components/Header';import {Sidebar} from './components/Sidebar';import {DriverMobileView} from './components/pwa/DriverMobileView';import {DriverToCentralRadioPanel} from './components/pwa/DriverToCentralRadioPanel';import {DriverPriorityCounter} from './components/pwa/DriverPriorityCounter';import {CentralRadioReceiver} from './components/radio/CentralRadioReceiver';
import {NewTripModal} from './components/modals/NewTripModal';import {SOSAlertModal} from './components/modals/SOSAlertModal';import {TripDetailModal} from './components/modals/TripDetailModal';import {VHFDispatchModal} from './components/modals/VHFDispatchModal';import {NotificationsDrawer} from './components/notifications/NotificationsDrawer';import {LoginScreen} from './components/auth/LoginScreen';import {OnboardingScreen} from './components/auth/OnboardingScreen';import {PartnerApplicationStatusScreen} from './components/auth/PartnerApplicationStatusScreen';import {DriverOnboardingPortal} from './components/driver/DriverOnboardingPortal';import {registerServiceWorker} from './lib/pwa';import {ErrorBoundary} from './components/system/ErrorBoundary';import {CommercialGate} from './components/system/CommercialGate';import {TrialGate} from './components/system/TrialGate';import {ArrowLeft,Loader2,ShieldAlert,ShieldCheck} from 'lucide-react';
...
const MODULE_ACCESS:Record<string,string[]>={dashboard:['super_admin','regional_partner','sales_partner','company_admin','operator'],network_centrals:['super_admin','regional_partner','sales_partner'],partners_network:['super_admin','regional_partner'],commissions_network:['super_admin','regional_partner','sales_partner'],plans_network:['super_admin','regional_partner','sales_partner'],network_support:['super_admin','regional_partner','sales_partner'],live_map:['super_admin'],trips:['company_admin','operator'],drivers:['company_admin','operator'],vehicles:['company_admin'],clients:['company_admin','operator'],operators:['company_admin'],companies:['super_admin'],users:['super_admin'],reports:['company_admin'],history:['super_admin','company_admin'],settings:['super_admin','company_admin'],profile:['super_admin','regional_partner','sales_partner','company_admin','operator'],help:['super_admin','regional_partner','sales_partner','company_admin','operator']};
...
const AuthenticatedShell:React.FC=()=>{const{session,authUser,loading,effectiveRole,profile,memberships,saasAccount,partnerApplication,identityError}=useAuth();const driverPath=location.pathname.startsWith('/driver');const needsPasswordSetup=Boolean(authUser?.user_metadata?.needs_password_setup);useEffect(()=>{registerServiceWorker();},[]);if(loading)return <main className="cg-auth-shell items-center justify-center"><div className="cg-card flex items-center gap-3 px-5 py-4"><Loader2 className="h-5 w-5 animate-spin text-[var(--cg-primary)]"/><span className="text-xs font-black text-[var(--cg-muted)]">Abriendo Central GO…</span></div></main>;if(!session)return <LoginScreen/>;if(needsPasswordSetup)return <PasswordSetupGate/>;const configured=Boolean(profile?.globalRole)||Boolean(saasAccount)||memberships.length>0;if(identityError&&!configured)return <IdentityErrorScreen message={identityError}/>;if(!configured)return <OnboardingScreen/>;if(saasAccount?.accountKind==='sales_partner'&&!profile?.globalRole)return <PartnerApplicationStatusScreen/>;if(partnerApplication?.status==='pending'&&!profile?.globalRole)return <PartnerApplicationStatusScreen/>;if(saasAccount?.accountKind==='driver'&&effectiveRole!=='driver')return <DriverOnboardingPortal/>;if(effectiveRole==='driver'&&!driverPath){location.replace('/driver');return null;}if(driverPath&&effectiveRole!=='driver')return <WrongDriverRoute/>;if(driverPath)return <CommercialAppProvider><div className="cg-operational"><DriverMobileView/><DriverPriorityCounter/><DriverToCentralRadioPanel/></div></CommercialAppProvider>;return <CommercialAppProvider><TrialGate><MainAppContent/></TrialGate></CommercialAppProvider>};
