...
interface HeaderProps {
  ...
  onToggleMenu: () => void;
  fireModeAvailable?: boolean;
  fireModeEnabled?: boolean;
...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="cg-topbar sticky top-0 z-40 w-full shrink-0 border-b border-zinc-800 bg-[#0d0d0f] px-3 py-2.5 shadow-xl md:px-6">
      <div className="mx-auto flex max-w-[1800px] items-center justify-between gap-3">
        <div className="flex min-w-0 items-center gap-3 md:gap-4">
          ...
          <div className="flex shrink-0 items-center gap-2.5">
            <img src={centralGoLogo} alt="Central GO" className="h-10 w-10 rounded-xl border border-blue-400/35 bg-zinc-950 p-0.5 shadow-lg shadow-blue-950/30" />
            <div className="hidden sm:block"><div className="flex items-center gap-1 text-lg font-black tracking-tight text-white">CENTRAL <span className="rounded-md border border-blue-400/35 bg-blue-500/15 px-1.5 py-0.5 text-blue-300">GO</span></div><p className="mt-0.5 text-[8px] font-black uppercase tracking-[0.16em] text-zinc-600">Movilidad conectada · datos sincronizados</p></div>
          </div>

          {isNetworkRole ? (
            <div className="hidden items-center gap-2 border-l border-zinc-800 pl-4 md:flex"><div className="rounded-lg border border-blue-500/20 bg-blue-500/10 p-1.5 text-blue-300">{currentRole === 'super_admin' ? <Globe2 className="h-3.5 w-3.5" /> : <MapPinned className="h-3.5 w-3.5" />}</div><div><p className="text-[8px] font-black uppercase tracking-widest text-zinc-600">Área de trabajo</p><p className="text-xs font-bold text-zinc-300">{currentRole === 'super_admin' ? 'Red Central GO' : roleLabel}</p></div></div>
          ) : (
            <div className="relative hidden border-l border-zinc-800 pl-4 md:block">
              <button onClick={() => setCompanyMenuOpen((open) => !open)} aria-expanded={companyMenuOpen} aria-haspopup="menu" className="flex items-center gap-2 rounded-lg border border-zinc-800 bg-zinc-900 px-3 py-1.5 text-xs font-semibold text-zinc-300 hover:bg-zinc-800"><Building2 className="h-3.5 w-3.5 text-blue-400" /><span className="max-w-[190px] truncate">{currentCompany.name}</span>{companies.length > 1 && <ChevronDown className="h-3 w-3 text-zinc-500" />}</button>
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          {!isNetworkRole && <button onClick={toggleSound} aria-pressed={!soundMuted} aria-label={soundMuted ? 'Activar sonidos' : 'Sonidos activos; pulsar para silenciar'} className={`flex items-center gap-1.5 rounded-lg border px-2.5 py-2 text-[10px] font-black ${soundMuted ? 'border-amber-400/30 bg-amber-400/10 text-amber-200' : 'border-blue-500/20 bg-blue-500/10 text-blue-300'}`} title={soundMuted ? 'Activar sonidos y reproducir una prueba' : 'Sonidos activos · pulsar para silenciar'}>{soundMuted ? <VolumeX className="h-4 w-4" /> : <Volume2 className="h-4 w-4" />}<span className="hidden xl:inline">{soundMuted ? 'Activar sonido' : 'Sonido activo'}</span></button>}
          ...
          {fireModeAvailable && <button type="button" onClick={onToggleFireMode} className="cg-fire-toggle" data-active={fireModeEnabled} aria-pressed={fireModeEnabled} title="Identidad exclusiva del Panel Global"><Flame className="h-4 w-4" /><span className="hidden xl:inline">Fuego {fireModeEnabled ? 'activo' : 'apagado'}</span></button>}
          <ThemeToggle compact />
          ...
          <div className="hidden items-center gap-2 rounded-lg border border-zinc-800 bg-zinc-900 px-2.5 py-1.5 md:flex"><div className={`h-2 w-2 rounded-full ${isNetworkRole ? 'bg-blue-400' : 'bg-emerald-400'}`} /><div className="max-w-[145px]"><p className="truncate text-[10px] font-black text-zinc-200">{profile?.name || roleLabel}</p><p className="truncate text-[8px] uppercase text-zinc-600">{roleLabel}</p></div>{currentRole === 'super_admin' && <ShieldCheck className="h-3.5 w-3.5 text-blue-300" />}</div>
