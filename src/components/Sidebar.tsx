    ...
    <>
      ...
      <aside aria-hidden={!isOpen} inert={!isOpen} className={`cg-sidebar fixed left-0 top-[61px] z-50 h-[calc(100vh-61px)] w-[min(88vw,300px)] overflow-y-auto border-r border-zinc-700 bg-[#0d0d0f] shadow-[24px_0_70px_rgba(0,0,0,.45)] transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className={currentRole === 'operator' ? 'space-y-2.5 p-2' : 'space-y-5 p-4'}>
          ...
          <nav className="space-y-1"><div className="mb-2 px-3 text-[9px] font-black uppercase tracking-widest text-zinc-600">{isNetworkRole ? 'Gestión comercial' : 'Operación'}</div>{allowedItems.map(({ id, label, icon: Icon }) => { const active=activeModule===id; return <button key={id} onClick={() => { setActiveModule(id); onClose(); }} className={`flex w-full items-center ${currentRole==='operator'?'gap-2.5 px-2.5 py-2 text-[11px]':'gap-3 px-3.5 py-2.5 text-xs'} rounded-lg text-left font-semibold transition ${active ? 'border border-blue-500/25 bg-blue-500/10 text-blue-300' : 'text-zinc-400 hover:bg-zinc-800/40 hover:text-zinc-200'}`}><Icon className={`h-4 w-4 ${active ? 'text-blue-300' : 'text-zinc-500'}`} /><span>{label}</span></button>; })}</nav>
