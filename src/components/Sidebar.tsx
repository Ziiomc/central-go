    ...
    <>
      ...
      <aside aria-hidden={!isOpen} inert={!isOpen} className={`cg-sidebar fixed left-0 top-0 z-50 h-[100dvh] w-[min(88vw,300px)] overflow-y-auto border-r border-zinc-700 bg-[#0d0d0f] shadow-[24px_0_70px_rgba(0,0,0,.45)] transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className={currentRole === 'operator' ? 'space-y-2.5 p-2' : 'space-y-5 p-4'}>
          ...
          <nav className="space-y-4">{groups.map((section) => <div key={section.name}><div className="mb-1.5 px-3 text-[8px] font-black uppercase tracking-[.18em] text-zinc-600">{section.name}</div><div className="space-y-1">{section.items.map(({ id, label, icon: Icon }) => { const active=activeModule===id; return <button key={id} onClick={() => { setActiveModule(id); onClose(); }} data-active={active} className={`cg-nav-item ${currentRole==='operator'?'is-compact':''}`}><span className="cg-nav-icon"><Icon className="h-4 w-4" /></span><span className="min-w-0 flex-1 truncate">{label}</span><span className="cg-nav-indicator" /></button>; })}</div></div>)}</nav>
