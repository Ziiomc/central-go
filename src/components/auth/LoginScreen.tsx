  ...
  return (
    <main className="min-h-screen bg-[#070709] text-zinc-100 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <section className="relative w-full max-w-md rounded-3xl border border-zinc-800 bg-[#0d0d0f]/95 p-7 sm:p-9 shadow-2xl shadow-black/60">
        ...
        <button type="button" disabled={busy} onClick={() => void login()} className="mt-6 flex w-full items-center justify-center gap-3 rounded-xl bg-white px-4 py-3.5 text-sm font-black text-zinc-950 transition hover:bg-zinc-100 disabled:opacity-60">
          {busy ? <Loader2 className="h-5 w-5 animate-spin" /> : <span className="grid h-5 w-5 place-items-center rounded-full bg-white text-base font-black text-blue-600">G</span>}{busy ? 'Conectando…' : 'Continuar con Google'}
        </button>
        <div className="mt-5 flex items-center justify-center gap-2 text-[10px] text-zinc-600"><ShieldCheck className="h-3.5 w-3.5" />Inicio seguro · tus datos permanecen protegidos</div>
      </section>
    </main>
  );
};