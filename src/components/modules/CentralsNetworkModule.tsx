  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4">
        <div><div className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.18em] text-blue-300 mb-2"><Globe2 className="w-3.5 h-3.5" />Red oficial sincronizada</div><h1 className="text-2xl md:text-3xl font-black text-white tracking-tight">{isSuper ? 'Centrales de la red' : 'Mis centrales'}</h1><p className="text-xs text-zinc-400 mt-1">Altas, plan, atribución comercial y estado operativo directamente desde Supabase.</p></div>
        <div className="flex gap-2"><button onClick={() => void reloadCentrals()} disabled={loading} className="px-4 py-3 rounded-xl bg-zinc-900 border border-zinc-800 text-xs font-bold text-zinc-300 flex items-center gap-2 disabled:opacity-50"><RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} />Actualizar</button><button onClick={() => setRegisterOpen(true)} className="px-5 py-3 rounded-xl bg-amber-500 hover:bg-amber-400 text-xs font-black text-slate-950 flex items-center justify-center gap-2"><Plus className="w-4 h-4" />Registrar central</button></div>
      </div>
      ...
      <CentralRegistrationModal open={registerOpen} onClose={() => setRegisterOpen(false)} onCreate={() => { void reloadCentrals(); }} />
    </div>
  );
};
