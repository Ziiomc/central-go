...
export const CentralsNetworkModule: React.FC = () => {
  ...
  const canRegister = isSuper || currentRole === 'sales_partner';
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4">
        <div><div className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.18em] text-blue-300 mb-2"><Globe2 className="w-3.5 h-3.5" />Red oficial sincronizada</div><h1 className="text-2xl md:text-3xl font-black text-white tracking-tight">{isSuper ? 'Centrales de la red' : 'Mis centrales'}</h1><p className="text-xs text-zinc-400 mt-1">{isSuper ? 'Registra nuevas empresas directamente, administra su prueba y define plan, modalidad u ofertas especiales.' : canRegister ? 'Registra y administra las centrales atribuidas automáticamente a tu código comercial.' : 'Consulta las centrales atribuidas a tu territorio.'}</p></div>
