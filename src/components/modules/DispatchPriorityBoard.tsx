...
import{ArrowDown,ArrowUp,Loader2,ShieldCheck,XCircle,Zap}from'lucide-react';
...
export const DispatchPriorityBoard:React.FC=()=>{
 ...
 return <section className="overflow-hidden rounded-2xl border border-zinc-800 bg-[#0d0d0f] shadow-xl shadow-black/20 xl:min-h-[650px]">
  ...
  <div className="p-3">
   ...
   {selectedTrip&&<div className="mt-2 rounded-xl border border-cyan-500/15 bg-cyan-500/[0.05] px-3 py-2"><div className="flex items-start gap-2"><div className="min-w-0 flex-1"><p className="truncate text-[10px] font-bold text-cyan-100">{selectedTrip.origin.address}</p><p className="mt-0.5 truncate text-[9px] text-zinc-500">→ {selectedTrip.destination.address}</p></div><button type="button" disabled={Boolean(busy)} onClick={()=>void cancelPendingTrip()} className="inline-flex shrink-0 items-center gap-1 rounded-lg border border-rose-500/30 bg-rose-500/10 px-2 py-1.5 text-[9px] font-black text-rose-300 disabled:opacity-40" title="Cancelar carrera pendiente"><XCircle className="h-3.5 w-3.5"/>{busy===`cancel-pending:${selectedTrip.id}`?<Loader2 className="h-3.5 w-3.5 animate-spin"/>:'Cancelar'}</button></div></div>}

   <div className="mt-2 max-h-[540px] space-y-1 overflow-y-auto pr-1">
    {connected.map((item,index)=>{const active=activeByDriver.get(item.driverId);return <article key={item.driverId} className="rounded-lg border border-zinc-800 bg-zinc-950/55 px-2 py-1.5">
     <div className="flex min-h-[48px] items-center gap-2">
      <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-cyan-500 text-[10px] font-black text-slate-950">#{index+1}</div>
      <strong className="min-w-0 flex-1 truncate text-[11px] text-white">{item.unitNumber} · {item.name}</strong>
      <div className="flex w-[92px] shrink-0 flex-col items-stretch gap-1">
       <span className={`self-center rounded-full px-2 py-0.5 text-[8px] font-black ${item.status==='available'?'bg-emerald-500/10 text-emerald-300':item.status==='in_trip'?'bg-blue-500/10 text-blue-300':'bg-amber-500/10 text-amber-300'}`}>{item.status==='available'?'Libre':item.status==='in_trip'?'En viaje':'Servicio'}</span>
       {active?<button disabled={Boolean(busy)} onClick={()=>void cancelDriverTrip(item,active)} className="flex h-6 min-w-0 items-center justify-center gap-1 rounded-md border border-rose-500/30 bg-rose-500/10 px-1.5 text-[8px] font-black text-rose-300 disabled:opacity-35"><XCircle className="h-3 w-3"/>Cancelar</button>:<button disabled={!selectedTrip||item.status!=='available'||Boolean(busy)} onClick={()=>void manualAssign(item)} className="flex h-6 min-w-0 items-center justify-center gap-1 rounded-md bg-cyan-500 px-1.5 text-[8px] font-black text-slate-950 disabled:opacity-35"><Zap className="h-3 w-3"/>Asignar carrera</button>}
      </div>
      <div className="flex shrink-0 gap-1">
       <button disabled={index===0||Boolean(busy)} onClick={()=>void move(item,'up')} className="grid h-7 w-7 place-items-center rounded-md border border-zinc-700 bg-zinc-900 text-zinc-300 disabled:opacity-25" title="Subir prioridad">{busy===`${item.driverId}:up`?<Loader2 className="h-3 w-3 animate-spin"/>:<ArrowUp className="h-3 w-3"/>}</button>
       <button disabled={index===connected.length-1||Boolean(busy)} onClick={()=>void move(item,'down')} className="grid h-7 w-7 place-items-center rounded-md border border-zinc-700 bg-zinc-900 text-zinc-300 disabled:opacity-25" title="Bajar prioridad">{busy===`${item.driverId}:down`?<Loader2 className="h-3 w-3 animate-spin"/>:<ArrowDown className="h-3 w-3"/>}</button>
      </div>
     </div>
    </article>})}
