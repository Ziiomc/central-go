 ...
 return <section className="overflow-hidden rounded-2xl border border-zinc-800 bg-[#0d0d0f] shadow-xl shadow-black/20">
  ...
  <div className="p-3">
   ...
   <div className="space-y-2">{connected.map((item,index)=>{const active=activeByDriver.get(item.driverId);const km=distanceFor(item);const real=item.routeDistanceKm!=null;const zone=km==null?'Sin GPS':km<3?'Cercano <3 km':km<=5?'Dentro de 5 km':`${km.toFixed(1)} km`;return <div key={item.driverId} className="grid items-center gap-2 rounded-xl border border-zinc-800 bg-zinc-950/55 p-2.5 md:grid-cols-[58px_minmax(150px,1fr)_minmax(105px,.7fr)_auto]">
    ...
    <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><strong className="text-xs text-white">{item.unitNumber} · {item.name}</strong><span className={`rounded-full px-2 py-0.5 text-[8px] font-black ${item.status==='available'?'bg-emerald-500/10 text-emerald-300':item.status==='in_trip'?'bg-blue-500/10 text-blue-300':'bg-amber-500/10 text-amber-300'}`}>{item.status==='available'?'Libre':item.status==='in_trip'?'En viaje':'En servicio'}</span></div><p className="mt-1 truncate text-[9px] text-zinc-500">{item.locationAddress}</p>{active&&<p className="mt-1 text-[9px] font-bold text-amber-300">Carrera activa: {active.code}</p>}</div>
    <div><p className="flex items-center gap-1 text-[10px] font-black text-zinc-300"><MapPin className="h-3.5 w-3.5 text-cyan-300"/>{zone}</p>{km!=null&&<p className={`mt-1 text-[8px] ${real?'font-bold text-emerald-400':'text-zinc-600'}`}>{real?`ruta vial real${item.routeDurationSeconds?` · ~${Math.max(1,Math.round(item.routeDurationSeconds/60))} min`:''}`:'respaldo GPS estimado'}</p>}</div>
