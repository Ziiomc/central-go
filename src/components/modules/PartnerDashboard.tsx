    ...
    return (
      <div className="space-y-6">
        <section className="rounded-3xl border border-amber-500/25 bg-amber-500/[0.05] p-7 md:p-10 text-center">
          ...
          <p className="mx-auto mt-2 max-w-xl text-sm leading-relaxed text-zinc-400">Tu cuenta tiene rol de partner, pero todavía no existe una ficha activa en el padrón comercial. Superadmin debe definir tu código, comisión y territorio antes de atribuir centrales.</p>
        </section>
    ...
    <div className="space-y-6">
      <section className="relative overflow-hidden rounded-3xl border border-blue-500/20 bg-gradient-to-br from-blue-950/35 via-[#0d0d0f] to-purple-950/25 p-6 md:p-8 shadow-2xl">
        <div className="relative flex flex-col xl:flex-row xl:items-end justify-between gap-6">
          <div className="max-w-2xl">
            ...
            <p className="text-sm text-zinc-400 mt-2">Gestiona tu cartera real, registra centrales y usa la comparación de planes para presentar una propuesta clara al cliente.</p>
      ...
      <section className="rounded-2xl border border-zinc-800 bg-[#0d0d0f] p-5 shadow-xl">
        ...
        <div className="mt-5 space-y-3">
          ...
          {!loading && (data?.centrals.length ?? 0) === 0 && <div className="p-10 text-center text-xs text-zinc-500">Todavía no tienes centrales atribuidas. Registra la primera desde este panel y quedará vinculada automáticamente a tu código.</div>}
        </div>
      </section>
      ...
      <CentralRegistrationModal open={registerOpen} onClose={() => setRegisterOpen(false)} onCreate={() => { void reload(); }} />
    </div>
  );
};
