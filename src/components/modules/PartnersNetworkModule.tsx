  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4">
        ...
        <div className="flex gap-2"><button onClick={() => void reload()} disabled={loading} className="px-4 py-3 rounded-xl bg-zinc-900 border border-zinc-800 text-xs font-bold text-zinc-300 flex items-center gap-2 disabled:opacity-50"><RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} />Actualizar</button>{isSuper && <button onClick={() => setInviteOpen(true)} className="px-5 py-3 rounded-xl bg-purple-600 hover:bg-purple-500 text-xs font-black text-white flex items-center gap-2"><UserPlus className="w-4 h-4" />Crear partner</button>}</div>
      </div>
      ...
      <section className="grid md:grid-cols-3 gap-4"><Info icon={MapPinned} title="Territorios claros" detail="Regional y comerciales quedan vinculados a una estructura real en la base." /><Info icon={BadgeDollarSign} title="Comisiones trazables" detail="Pendiente, disponible y pagada se separan por estado contable." /><Info icon={ShieldCheck} title="Accesos personales" detail="Cada partner recibe una invitación y define su propia contraseña." /></section>

      {inviteOpen && isSuper && <InvitePartnerModal partners={partners} onClose={() => setInviteOpen(false)} onDone={(message) => { setNotice(message); setInviteOpen(false); void reload(); }} onError={setError} />}
    </div>
  );
};
...
const InvitePartnerModal: React.FC<{ partners: PartnerDirectoryItem[]; onClose: () => void; onDone: (message: string) => void; onError: (message: string) => void }> = ({ partners,onClose,onDone,onError }) => {
  ...
  const submit=async(e:React.FormEvent)=>{ e.preventDefault(); setSaving(true); onError(''); try { const result=await inviteNetworkPartner({ name:form.name,email:form.email,kind:form.kind,code:form.code,commissionPercent:Number(form.commission),parentPartnerId:form.kind==='sales'?(form.parentPartnerId||null):null,countryCode:form.countryCode,region:form.region,city:form.city,redirectTo:`${window.location.origin}/` }); onDone(result.message); } catch(err){ onError(err instanceof Error?err.message:'No fue posible crear el partner.'); } finally{ setSaving(false); } };
  return <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/80 p-4 backdrop-blur-md"><form onSubmit={submit} className="w-full max-w-xl rounded-3xl border border-zinc-700 bg-[#0d0d0f] p-6 shadow-2xl"><div className="flex items-start justify-between gap-3"><div><p className="text-[9px] font-black uppercase tracking-widest text-purple-300">Programa Internacional de Partners</p><h2 className="mt-2 text-xl font-black text-white">Crear e invitar partner</h2><p className="mt-1 text-xs text-zinc-500">La cuenta y el perfil comercial se crean en una sola operación.</p></div><button type="button" onClick={onClose} className="rounded-xl border border-zinc-800 bg-zinc-900 p-2 text-zinc-400"><X className="h-4 w-4" /></button></div><div className="mt-5 grid gap-4 sm:grid-cols-2"><Field label="Nombre" value={form.name} onChange={(v)=>setForm((p)=>({...p,name:v}))} /><Field label="Correo" type="email" value={form.email} onChange={(v)=>setForm((p)=>({...p,email:v}))} /><label className="block"><span className="mb-1.5 block text-[10px] font-black uppercase text-zinc-500">Tipo</span><select value={form.kind} onChange={(e)=>changeKind(e.target.value as 'regional'|'sales')} className="w-full rounded-xl border border-zinc-800 bg-zinc-950 px-3 py-3 text-sm text-white"><option value="regional">Partner regional</option><option value="sales">Partner comercial</option></select></label><Field label="Código" value={form.code} onChange={(v)=>setForm((p)=>({...p,code:v.toUpperCase().replace(/[^A-Z0-9-]/g,'')}))} /><Field label="Comisión %" type="number" value={form.commission} onChange={(v)=>setForm((p)=>({...p,commission:v}))} />{form.kind==='sales'&&<label className="block"><span className="mb-1.5 block text-[10px] font-black uppercase text-zinc-500">Regional responsable</span><select value={form.parentPartnerId} onChange={(e)=>setForm((p)=>({...p,parentPartnerId:e.target.value}))} className="w-full rounded-xl border border-zinc-800 bg-zinc-950 px-3 py-3 text-sm text-white"><option value="">Sin regional</option>{regionals.map((p)=><option key={p.id} value={p.id}>{p.name}</option>)}</select></label>}<Field label="País (código)" value={form.countryCode} onChange={(v)=>setForm((p)=>({...p,countryCode:v.toUpperCase().slice(0,2)}))} /><Field label="Región" value={form.region} onChange={(v)=>setForm((p)=>({...p,region:v}))} /><Field label="Ciudad" value={form.city} onChange={(v)=>setForm((p)=>({...p,city:v}))} /><div className="sm:col-span-2 rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-3 text-[10px] leading-relaxed text-emerald-200/80">El correo recibirá una invitación para activar la cuenta y definir su contraseña. La comisión corresponde a ventas de centrales, no al reclutamiento de otros partners.</div></div><button disabled={saving} className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-purple-600 px-4 py-3 text-xs font-black text-white disabled:opacity-50">{saving?<Loader2 className="h-4 w-4 animate-spin"/>:<Plus className="h-4 w-4"/>}{saving?'Creando partner…':'Crear partner y enviar invitación'}</button></form></div>;
};
