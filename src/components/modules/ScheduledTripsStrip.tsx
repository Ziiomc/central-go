...
import{loadReservableDriverIds,reserveScheduledTripAtomic}from'../../lib/reservationRepository';
...
export const ScheduledTripsStrip:React.FC=()=>{
 ...
 const[reservableDriverIds,setReservableDriverIds]=useState<Set<string>|null>(null);
 const driverEligibilityKey=useMemo(()=>drivers.map(driver=>driver.id).sort().join('|'),[drivers]);
 ...
 useEffect(()=>{
  ...
  const refresh=async()=>{
   ...
   running=true;
   try{
    const ids=await loadReservableDriverIds(drivers.map(driver=>driver.id));
    if(active)setReservableDriverIds(ids);
   }catch(error){
    console.warn('[Central GO] reservable drivers unavailable',error);
    if(active)setReservableDriverIds(null);
 ...
 const reservationDrivers=useMemo(()=>drivers
  .filter(driver=>driver.status!=='sos'&&(reservableDriverIds===null||reservableDriverIds.has(driver.id)))
  .slice()
  .sort((a,b)=>a.unitNumber.localeCompare(b.unitNumber,'es',{numeric:true})),[drivers,reservableDriverIds]);
 ...
 return <section className="overflow-hidden rounded-2xl border border-zinc-800 bg-[#0d0d0f] shadow-xl shadow-black/20">
  ...
  <div className="max-h-[calc(100vh-210px)] divide-y divide-zinc-800/80 overflow-y-auto">{scheduled.map(trip=>{const remaining=minutesUntil(trip.scheduledFor!);const automatic=trip.dispatchMode==='automatic';const selected=driverChoice[trip.id]??trip.reservedDriverId??'';const today=sameLocalDay(trip.scheduledFor!);const inDispatchWindow=remaining<=20;const reservedDriverDisabled=Boolean(trip.reservedDriverId&&reservableDriverIds&&!reservableDriverIds.has(trip.reservedDriverId));return <article key={trip.id} className="p-3.5 hover:bg-zinc-900/45">
   ...
   <div className="mt-3 flex flex-wrap items-center gap-1.5 rounded-xl border border-violet-500/15 bg-violet-500/[0.04] p-2"><span className="mr-1 text-[8px] font-black uppercase tracking-wider text-violet-300">Móvil reservado</span><select value={selected} onChange={event=>setDriverChoice(current=>({...current,[trip.id]:event.target.value}))} className="h-8 min-w-[170px] flex-1 rounded-lg border border-zinc-700 bg-zinc-950 px-2 text-[9px] font-bold text-zinc-200 outline-none"><option value="">Sin móvil específico</option>{reservedDriverDisabled&&trip.reservedDriverId&&<option value={trip.reservedDriverId} disabled>Móvil {trip.reservedDriverUnitNumber??'reservado'} · deshabilitado</option>}{reservationDrivers.map(driver=><option key={driver.id} value={driver.id}>Móvil {driver.unitNumber} · {driver.name} · {driver.operationMode==='traditional'?'Radio':'App'}</option>)}</select><button type="button" disabled={!selected||reservedDriverDisabled||busyId===trip.id} onClick={()=>void reserveDriver(trip)} className="h-8 rounded-lg bg-violet-600 px-2.5 text-[8px] font-black text-white disabled:opacity-40">{trip.reservedDriverId?'Cambiar':'Reservar'}</button>{trip.reservedDriverId&&<button type="button" disabled={busyId===trip.id} onClick={()=>void clearReservedDriver(trip)} className="grid h-8 w-8 place-items-center rounded-lg border border-violet-500/20 bg-zinc-950 text-violet-300 disabled:opacity-40" title="Quitar móvil reservado"><XCircle className="h-3.5 w-3.5"/></button>}</div>
  </article>})}</div>}
 </section>;
};
