...
export const CentralRegistrationModal: React.FC<CentralRegistrationModalProps> = ({ open, onClose, onCreate }) => {
  ...
  const [success, setSuccess] = useState<{ ownerInvited: boolean; ownerAssigned: boolean; passwordReady: boolean } | null>(null);
  const [form, setForm] = useState({
    name: '', code: '', country: 'Chile', region: '', city: '', owner: '', phone: '', email: '', password: '', confirmPassword: '', vehicles: '20', plan: 'Enterprise' as 'Start' | 'Pro' | 'Enterprise',
  });
  ...
  const passwordValid = currentRole !== 'super_admin' || (form.password.length >= 10 && form.password === form.confirmPassword);
  const canContinue = step === 1
    ? form.name.trim().length >= 2 && form.code.trim().length >= 2 && form.city.trim().length >= 2
    : step === 2
      ? form.owner.trim().length >= 2 && form.email.includes('@') && passwordValid
  ...
  const resetAndClose = () => {
    ...
    setForm({ name: '', code: '', country: 'Chile', region: '', city: '', owner: '', phone: '', email: '', password: '', confirmPassword: '', vehicles: '20', plan: 'Enterprise' });
    onClose();
  };

  const submit = async () => {
    if (!selectedPlan) return;
    if (currentRole === 'super_admin' && (!passwordValid || form.password.length < 10)) {
      setError('La contraseña inicial debe tener al menos 10 caracteres y ambas contraseñas deben coincidir.');
      return;
    }
    ...
    try {
      ...
      if (form.email.trim() && (!ownerAssigned || currentRole === 'super_admin')) {
        const access = await inviteCompanyUser({
          companyId: result.companyId,
          email: form.email,
          name: form.owner,
          role: 'company_admin',
          redirectTo: 'https://central-go-one.vercel.app/',
          initialPassword: currentRole === 'super_admin' ? form.password : undefined,
        });
        ownerAssigned = true;
        ownerInvited = access.invited;
        passwordReady = access.passwordReady;
      }
      ...
      setSuccess({ ownerInvited, ownerAssigned, passwordReady });
    } catch (err) {
    ...
    <div className="fixed inset-0 z-[90] overflow-y-auto bg-black/80 p-0 backdrop-blur-md sm:flex sm:items-center sm:justify-center sm:p-4">
      <div className="flex min-h-full w-full max-w-4xl flex-col overflow-visible rounded-none border border-zinc-700 bg-[#0d0d0f] shadow-2xl sm:min-h-0 sm:max-h-[calc(100dvh-2rem)] sm:overflow-hidden sm:rounded-3xl">
        <div className="shrink-0 flex items-start justify-between gap-4 border-b border-zinc-800 bg-gradient-to-r from-purple-950/30 via-[#0d0d0f] to-blue-950/20 p-5 sm:p-6">
          <div><div className="mb-2 inline-flex items-center gap-2 rounded-full border border-purple-500/20 bg-purple-500/10 px-2.5 py-1 text-[10px] font-black uppercase tracking-widest text-purple-300"><Zap className="h-3 w-3" />Alta oficial</div><h2 className="text-xl font-black text-white">Registrar nueva central</h2><p className="mt-1 text-xs text-zinc-400">Crea la empresa, activa su prueba de 5 días y deja listo el acceso de su administrador.</p></div>
        ...
        {success ? (
          <div className="p-8 text-center sm:p-10"><div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl border border-emerald-500/25 bg-emerald-500/10 text-emerald-300"><MailCheck className="h-7 w-7" /></div><h3 className="mt-5 text-xl font-black text-white">Central creada correctamente</h3><p className="mx-auto mt-2 max-w-lg text-sm leading-relaxed text-zinc-400">La central quedó registrada con 5 días de prueba. {success.passwordReady ? 'El administrador ya puede iniciar sesión con el correo y la contraseña que definiste.' : success.ownerInvited ? 'Enviamos una invitación al administrador para completar su acceso.' : success.ownerAssigned ? 'La cuenta existente del administrador quedó vinculada a la central.' : 'El acceso del administrador queda pendiente.'}</p><button onClick={resetAndClose} className="mt-6 rounded-xl bg-emerald-500 px-5 py-3 text-xs font-black text-zinc-950 hover:bg-emerald-400">Continuar</button></div>
        ) : (
          <>
