...
export const DriverPriorityCounter:React.FC=()=>{
 ...
 const load=async()=>{
  if(currentRole!=='driver'||currentCompany.id==='network')return;
  try{
   const next=await loadDriverQueueSnapshot(currentCompany.id);
   setQueue(next);
   setHasLoaded(true);
  }catch{
   // A temporary mobile/network interruption must never replace a valid server
   // snapshot with an error state. Keep showing the last confirmed queue.
  }
 };
 ...
 const positionLabel=!hasLoaded?'—':inQueue?String(position+1):'—';
 const driversLabel=!hasLoaded?'—':String(connected.length);
 ...
 return <>
  ...
  <section data-driver-priority-card="1" className="overflow-hidden rounded-xl border border-slate-600/45 bg-[#0d1117] shadow-lg" aria-label={`Tu lugar en la fila: ${positionLabel}. Conductores conectados: ${driversLabel}`}>
   <div className="grid grid-cols-2">
    <div data-driver-priority-cell="1" className="flex min-h-[62px] items-center gap-2 border-r border-slate-700/70 px-3 py-2">
     <div data-driver-priority-icon="position" className={`grid h-7 w-7 shrink-0 place-items-center rounded-lg border ${inQueue?'border-blue-400/25 bg-blue-500/15 text-blue-200':'border-slate-700 bg-slate-800 text-slate-300'}`}><Hash className="h-3.5 w-3.5"/></div>
   ...
   {colleaguesOpen&&<div data-driver-priority-list="1" className="border-t border-slate-700/70 bg-[#10161f]">
    <div className="max-h-52 divide-y divide-slate-700/60 overflow-y-auto">
     ...
     {hasLoaded&&!connected.length?<p data-driver-priority-empty="1" className="px-3 py-3 text-center text-[9px] font-semibold text-slate-300">No hay otros móviles conectados.</p>:null}
    </div>
   </div>}
  </section>
 </>;
