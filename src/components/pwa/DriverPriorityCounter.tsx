...
export const DriverPriorityCounter:React.FC=()=>{
 ...
 const value=error?'…':inQueue?`${position+1}-${connected.length}`:'—';
 return <>
  <DriverTripCancellationControl/>
  {own&&<div className="pointer-events-none fixed top-[calc(env(safe-area-inset-top)+.72rem)] z-[145]" style={{right:'max(4.05rem, calc((100vw - 28rem)/2 + 4.15rem))'}}>
   <div className={`flex h-10 min-w-[72px] items-center justify-center rounded-xl border px-2.5 shadow-md backdrop-blur-xl ${error?'border-rose-400/35 bg-rose-950/90 text-rose-200':inQueue?'border-cyan-400/35 bg-[var(--cg-surface-solid)] text-[var(--cg-text)]':'border-[var(--cg-border)] bg-[var(--cg-surface-solid)] text-[var(--cg-muted)]'}`}>
