...
export const DriverQueueInlineCard:React.FC=()=>{
 ...
 const load=async()=>{
  ...
  try{setQueue(await loadDriverQueueSnapshot(currentCompany.id));setError(false);}catch{setError(true);}
 };

 useEffect(()=>{
  void load();
  if(currentRole!=='driver'||currentCompany.id==='network')return;
  const unsubscribe=subscribeDispatchQueue(currentCompany.id,()=>void load());
  const timer=window.setInterval(()=>void load(),30000);
  return()=>{unsubscribe();window.clearInterval(timer);};
 },[currentRole,currentCompany.id,currentUser.id]);
 ...
 return <section className={`overflow-hidden rounded-xl border shadow-lg ${error?'border-rose-400/30 bg-[#191014]':'border-slate-600/45 bg-[#0d1117]'}`} aria-label={`Tu lugar en la fila: ${positionLabel}. Conductores conectados: ${driversLabel}`}>
  ...
  {colleaguesOpen&&<div className="border-t border-slate-700/70 bg-[#10161f]">
   <div className="max-h-56 divide-y divide-slate-700/60 overflow-y-auto">
    ...
    {error?<div className="px-3 py-3 text-center"><p className="text-[10px] font-semibold text-rose-200">No pudimos actualizar la fila.</p><button type="button" onClick={()=>void load()} className="mt-2 min-h-9 rounded-lg border border-slate-600 bg-slate-800 px-3 text-[9px] font-black text-white">Reintentar</button></div>:null}
   </div>
  </div>}
 </section>;
};
