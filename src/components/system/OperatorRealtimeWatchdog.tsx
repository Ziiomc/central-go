...
export const OperatorRealtimeWatchdog:React.FC=()=>{
 ...
 useEffect(()=>{
  ...
  if(fresh&&!soundMuted)void soundManager.prime().then(ready=>{
   if(!ready)return;
   if(fresh.scheduledFor)soundManager.playReservationAlarm();
   else soundManager.playDispatchChime();
  });
 },[trips,currentRole,soundMuted]);

 useEffect(()=>{if(!['operator','company_admin'].includes(currentRole))return;let stopped=false;const paint=()=>{if(stopped)return;const now=Date.now(),light=document.documentElement.dataset.theme==='light',byId=new Map(tripsRef.current.map(t=>[t.id,t]));document.querySelectorAll<HTMLElement>('[data-dispatch-trip-id]').forEach(node=>{const trip=node.dataset.dispatchTripId?byId.get(node.dataset.dispatchTripId):undefined;if(!trip){clearPendingAgeStyle(node);return;}if(trip.scheduledFor){node.style.setProperty('background-color',light?'rgba(207,250,254,.96)':'rgba(8,145,178,.16)','important');node.style.setProperty('box-shadow','inset 4px 0 0 #22d3ee, inset 0 0 0 1px rgba(34,211,238,.26)','important');node.style.setProperty('border-color','rgba(34,211,238,.46)','important');node.setAttribute(AGE_MARK,'cyan');return;}if(trip.status!=='pending'){clearPendingAgeStyle(node);return;}const age=now-new Date(trip.createdAt).getTime();if(age>=RED_AFTER_MS){node.style.setProperty('background-color',light?'rgba(254,226,226,.96)':'rgba(127,29,29,.32)','important');node.style.setProperty('box-shadow','inset 4px 0 0 #ef4444, inset 0 0 0 1px rgba(239,68,68,.35)','important');node.style.setProperty('border-color','rgba(239,68,68,.55)','important');node.setAttribute(AGE_MARK,'red');}else if(age>=WARNING_AFTER_MS){node.style.setProperty('background-color',light?'rgba(254,249,195,.98)':'rgba(120,53,15,.30)','important');node.style.setProperty('box-shadow','inset 4px 0 0 #eab308, inset 0 0 0 1px rgba(234,179,8,.38)','important');node.style.setProperty('border-color','rgba(202,138,4,.48)','important');node.setAttribute(AGE_MARK,'yellow');}else clearPendingAgeStyle(node);});};paint();const interval=window.setInterval(paint,1000);return()=>{stopped=true;window.clearInterval(interval);document.querySelectorAll<HTMLElement>(`[${AGE_MARK}]`).forEach(clearPendingAgeStyle);};},[currentCompany.id,currentRole]);
