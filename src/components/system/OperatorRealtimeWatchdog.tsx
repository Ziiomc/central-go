...
export const OperatorRealtimeWatchdog:React.FC=()=>{
 ...
 useEffect(()=>{
  ...
  if(fresh&&!soundMuted)void soundManager.prime().then(ready=>{
   if(!ready)return;
   if(fresh.scheduledFor)soundManager.playReservationAlarmOnce(`created:${fresh.id}`);
   else soundManager.playDispatchChime();
  });
 },[trips,currentRole,soundMuted]);

 useEffect(()=>{if(currentCompany.id==='network'||!['operator','company_admin'].includes(currentRole)||soundMuted)return;let stopped=false;const check=()=>{if(stopped)return;const now=synchronizedNow();const due=tripsRef.current.filter(trip=>trip.scheduledFor&&!['completed','cancelled'].includes(trip.status)&&tripDelayMs(trip,now)>=-RESERVATION_ALARM_WINDOW_MS&&tripDelayMs(trip,now)<30*60*1000);if(!due.length)return;void soundManager.prime().then(ready=>{if(!ready||stopped)return;due.forEach(trip=>soundManager.playReservationAlarmOnce(`due:${trip.id}`));});};check();const timer=window.setInterval(check,5000);return()=>{stopped=true;window.clearInterval(timer);};},[currentCompany.id,currentRole,soundMuted]);
