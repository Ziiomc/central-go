...
export const AuthProvider:React.FC<React.PropsWithChildren>=({children})=>{
 ...
 const loadIdentity=async(nextSession:Session|null)=>{
  ...
  const hadStableIdentity=Boolean(nextSession&&identityUserRef.current===nextSession.user.id);
  setStableSession(nextSession);setIdentityError(null);
  ...
  try{
   const readProfile=()=>db.from('profiles').select('id,name,phone,avatar_url,global_role,active').eq('id',nextSession.user.id).single();
   let profileResult=await readProfile();
   if(profileResult.error){await wait(350);if(requestId!==identityRequestRef.current)return;profileResult=await readProfile();}
   ...
   const readMemberships=()=>db.from('company_memberships').select('company_id,role,active,companies(id,name,code,phone,address,vhf_frequency,logo_url,active)').eq('user_id',nextSession.user.id).eq('active',true);
   let membershipResult=await readMemberships();
   if(membershipResult.error){await wait(300);if(requestId!==identityRequestRef.current)return;membershipResult=await readMemberships();}
   if(requestId!==identityRequestRef.current)return;
   const {data:membershipRows,error:membershipError}=membershipResult;
   let mapped:Membership[]=[];
   if(membershipError){if(!nextProfile.globalRole)throw membershipError;console.warn('[Central GO] memberships unavailable',membershipError);setIdentityError('Tu cuenta está activa, pero no pudimos cargar temporalmente sus accesos de central.');}
   else mapped=(membershipRows??[]).filter((r:any)=>r.companies).map((r:any)=>({companyId:r.company_id,role:r.role,active:r.active,company:mapCompany(Array.isArray(r.companies)?r.companies[0]:r.companies)}));
   ...
   if(saasResult.error){console.warn('[Central GO] saas account unavailable',saasResult.error);setSaasAccount(null);if(!nextProfile.globalRole&&mapped.length===0)setIdentityError('Central GO está reconectando tu cuenta. Tu sesión continúa iniciada.');}else{const row=saasResult.data;setSaasAccount(row?{accountKind:row.account_kind,companyId:row.company_id,status:row.status,trialStartedAt:row.trial_started_at,trialEndsAt:row.trial_ends_at,currentPeriodEnd:row.current_period_end,countryCode:row.country_code,region:row.region,city:row.city}:null);}
  }catch(error){
   if(requestId!==identityRequestRef.current)return;
   console.warn('[Central GO] temporary identity sync unavailable',error);
   if(hadStableIdentity){
    // Mobile browsers commonly suspend/resume networking. A temporary refresh
    // failure must never erase an already validated driver/operator identity.
    setIdentityError(null);
    return;
   }
   identityUserRef.current=null;
   setIdentityError('Central GO está reconectando tu sesión. Tu acceso sigue iniciado; vuelve a intentar en unos segundos.');
   setProfile(null);setMemberships([]);setCompanies([]);setSaasAccount(null);setDriverApplication(null);setPartnerApplication(null);
  }finally{if(requestId===identityRequestRef.current)setLoading(false);}
 };
 ...
 useEffect(()=>{
  ...
  const bootstrap=async()=>{
   try{
    ...
    if(sessionResult.error){await wait(450);if(!mounted)return;sessionResult=await supabase.auth.getSession();}
    if(!mounted)return;
    if(sessionResult.error){setIdentityError('Central GO está recuperando tu sesión. Vuelve a intentar en unos segundos.');setLoading(false);return;}
    await loadIdentity(sessionResult.data.session);
   }catch(error){if(!mounted)return;console.warn('[Central GO] session bootstrap unavailable',error);setIdentityError('Central GO está recuperando tu sesión. Vuelve a intentar en unos segundos.');setLoading(false);}
  };
  void bootstrap();
  const {data:listener}=supabase.auth.onAuthStateChange((event,next)=>{const timer=window.setTimeout(()=>{if(mounted)handleAuthChange(event,next);},0);timers.push(timer);});
  const resume=()=>{
   if(document.visibilityState!=='visible')return;
   void supabase.auth.getSession().then(({data,error})=>{
    if(!mounted||error||!data.session)return;
    setStableSession(data.session);
    if(identityUserRef.current!==data.session.user.id)void loadIdentity(data.session);
   }).catch(()=>undefined);
  };
 ...
 const signInWithGoogle=async(intent?:OnboardingRole)=>{rememberIntent(intent);const db=requireSupabase();const {error}=await db.auth.signInWithOAuth({provider:'google',options:googleOAuthOptions(`${runtimeConfig.officialAppUrl}/`)});if(error)throw error;};
