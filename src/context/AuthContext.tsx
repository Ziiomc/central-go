...
interface AuthContextValue { session:Session|null; authUser:SupabaseUser|null; profile:AuthProfile|null; memberships:Membership[]; companies:Company[]; saasAccount:SaaSAccount|null; effectiveRole:UserRole|null; loading:boolean; identityError:string|null; signInWithGoogle:()=>Promise<void>; signOut:()=>Promise<void>; refreshIdentity:()=>Promise<void>; }
...
export const AuthProvider:React.FC<React.PropsWithChildren>=({children})=>{
 ...
 const value=useMemo(()=>({session,authUser:session?.user??null,profile,memberships,companies,saasAccount,effectiveRole,loading,identityError,signInWithGoogle,signOut,refreshIdentity:()=>loadIdentity(session)}),[session,profile,memberships,companies,saasAccount,effectiveRole,loading,identityError]);
