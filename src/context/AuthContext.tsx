...
interface AuthContextValue { session:Session|null; authUser:SupabaseUser|null; profile:AuthProfile|null; memberships:Membership[]; companies:Company[]; saasAccount:SaaSAccount|null; effectiveRole:UserRole|null; loading:boolean; identityError:string|null; signInWithGoogle:()=>Promise<void>; signUp:(email:string,password:string)=>Promise<boolean>; signIn:(email:string,password:string)=>Promise<void>; signOut:()=>Promise<void>; updatePassword:(password:string)=>Promise<void>; requestPasswordReset:(email:string)=>Promise<void>; refreshIdentity:()=>Promise<void>; }
...
export const AuthProvider:React.FC<React.PropsWithChildren>=({children})=>{
 ...
 const loadIdentity=async(nextSession:Session|null)=>{
  ...
  try{
   ...
   if(nextProfile.globalRole==='super_admin'){setMemberships([]);setSaasAccount(null);const {data,error}=await db.from('companies').select('id,name,code,phone,address,vhf_frequency,logo_url,active').order('name');if(requestId!==identityRequestRef.current)return;if(error){console.warn('[Central GO] companies unavailable for superadmin',error);setCompanies([]);setIdentityError('Tu cuenta Superadmin está activa, pero no pudimos cargar las centrales. Pulsa actualizar o vuelve a intentar.');}else setCompanies((data??[]).map(mapCompany));return;}
   ...
   const readSaas=()=>db.from('saas_accounts').select('account_kind,company_id,status,trial_started_at,trial_ends_at,current_period_end').eq('user_id',nextSession.user.id).maybeSingle();let saasResult=await readSaas();if(saasResult.error){await wait(300);if(requestId!==identityRequestRef.current)return;saasResult=await readSaas();}if(requestId!==identityRequestRef.current)return;
   if(saasResult.error){console.warn('[Central GO] saas account unavailable',saasResult.error);setSaasAccount(null);if(!nextProfile.globalRole&&mapped.length===0)setIdentityError('No pudimos validar temporalmente tu tipo de cuenta. Reintenta antes de crear una cuenta nueva.');}else{const row=saasResult.data;setSaasAccount(row?{accountKind:row.account_kind,companyId:row.company_id,status:row.status,trialStartedAt:row.trial_started_at,trialEndsAt:row.trial_ends_at,currentPeriodEnd:row.current_period_end}:null);}
  }catch(error){if(requestId!==identityRequestRef.current)return;console.error('[Central GO] identity',error);identityUserRef.current=null;setIdentityError(error instanceof Error?error.message:'No fue posible cargar el perfil.');setProfile(null);setMemberships([]);setCompanies([]);setSaasAccount(null);}finally{if(requestId===identityRequestRef.current)setLoading(false);}
 };
 ...
 const signUp=async(email:string,password:string)=>{const normalized=email.trim().toLowerCase();if(!normalized.includes('@'))throw new Error('Ingresa un correo válido.');if(password.length<10)throw new Error('La contraseña debe tener al menos 10 caracteres.');const db=requireSupabase();const {data,error}=await db.auth.signUp({email:normalized,password,options:{emailRedirectTo:`${runtimeConfig.officialAppUrl}/`,data:{account_kind:'sales_partner'}}});if(error)throw error;if(data.session){await loadIdentity(data.session);return true;}return false;};
