...
export const AuthProvider:React.FC<React.PropsWithChildren>=({children})=>{
 ...
 const signInWithGoogle=async()=>{const db=requireSupabase();const {error}=await db.auth.signInWithOAuth({provider:'google',options:{redirectTo:'https://go-one.vercel.app/'}});if(error)throw error;};
 ...
 const requestPasswordReset=async(email:string)=>{const normalized=email.trim().toLowerCase();if(!normalized.includes('@'))throw new Error('Ingresa un correo válido.');const db=requireSupabase();const {error}=await db.auth.resetPasswordForEmail(normalized,{redirectTo:'https://go-one.vercel.app/'});if(error)throw error;};
