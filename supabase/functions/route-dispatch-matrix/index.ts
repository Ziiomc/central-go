...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  if(trip.status!=="pending"||trip.dispatch_mode!=="automatic")return json({ok:true,skipped:true,status:trip.status});
  ...
  const points:DriverPoint[]=(locations??[])
  ...
  for(let i=0;i<points.length;i+=38)chunks.push(points.slice(i,i+38));
  if(!chunks.length)chunks.push([]);

  await db.from("trip_driver_route_metrics").delete().eq("trip_id",tripId);
  ...
  for(const chunk of chunks){
   ...
   try{
    const response=await fetch(endpoint,{headers:{Accept:"application/json","User-Agent":"CentralGO/2.0 routing-matrix"},signal:AbortSignal.timeout(2200)});
  ...
  if(tripDistanceKm!=null){
   ...
   }).eq("id",tripId).eq("status","pending");
   if(error)throw error;
  }

  const {error:dispatchError}=await db.rpc("centralgo_internal_dispatch_trip",{p_trip_id:tripId});
  if(dispatchError)throw dispatchError;
  return json({ok:true,tripId,provider:successfulChunks?"osrm-road":"fallback",driverRoutes:metricRows.length,tripDistanceKm,tripDurationSeconds,successfulChunks,totalChunks:chunks.length});
 }catch(error){
  console.error("route-dispatch-matrix",error);
  if(tripId){try{await db.rpc("centralgo_internal_dispatch_trip",{p_trip_id:tripId});}catch{}}
  return json({error:error instanceof Error?error.message:"No fue posible calcular la matriz vial",fallbackDispatch:true},200);
 }
});
