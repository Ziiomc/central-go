-- Keep the registration location separate from assigned sales territories while
-- exposing it through the already-authorized partner directory RPC.
...
begin
  ...
  return (
    select coalesce(jsonb_agg(jsonb_build_object(
      ...
      'registrationCountryCode',registration.country_code,
    ...
    left join lateral (
      select pa.country_code,pa.region,pa.city
      from public.partner_applications pa
      where pa.status='approved'
        and (pa.partner_id=p.id or pa.user_id=p.user_id)
      order by case when pa.partner_id=p.id then 0 else 1 end,
    ...
    left join lateral (
      select count(*)::int as central_count,
             ...
             end else 0 end),0) as monthly_sales
    ...
    left join lateral (
      ...
      where cl.partner_id=p.id
    ) comm on true
    where p.archived_at is null
      and (is_super or p.id=viewer_partner or p.parent_partner_id=viewer_partner)
  );
end;
$$;
