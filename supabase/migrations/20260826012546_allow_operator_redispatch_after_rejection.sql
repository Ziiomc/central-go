-- Keep manual redispatch reliable after a driver rejects or an operator removes an offer.
-- This migration was applied to production first; keeping it in Git removes schema drift.
...
grant execute on function public.centralgo_operator_unassign_trip(uuid,text) to authenticated,service_role;
