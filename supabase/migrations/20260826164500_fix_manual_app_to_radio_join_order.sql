-- Ensure an App driver explicitly incorporated by the operator into radio mode
-- joins at the end of the current dispatch queue, even when its stale driver row
-- still says service_enabled=true/status=available.
